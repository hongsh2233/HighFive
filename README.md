...
- **업무 관리**: 생성/배정/수정/삭제, 제목에서 RMS 번호 자동 파싱, 상태 전이(기본 `ASSIGNED → PROGRESS → REVIEW → QA → DONE`, 프로젝트별로 단계 커스터마이징 가능)
- **칸반 보드**: 프로젝트 선택 시 해당 프로젝트의 커스텀 단계로 컬럼 구성, 드래그 앤 드롭으로 상태 변경
...
- **프로젝트 관리**: 프로젝트 단위로 업무 묶기, 프로젝트 멤버 관리, 프로젝트별 업무 상태(칸반 단계) 자유 구성(`/projects/[id]/statuses`)
