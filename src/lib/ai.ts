...
export type LlmProvider = 'ANTHROPIC' | 'OPENAI' | 'GEMINI' | 'GROQ' | 'NVIDIA';
...
async function callNvidia(prompt: string, maxTokens: number, apiKey?: string | null): Promise<string> {
  const key = apiKey || process.env.NVIDIA_API_KEY;
  if (!key) {
    throw new Error('AI 기능을 사용하려면 조직 설정(AI 설정)에 NVIDIA NIM API 키를 등록해야 합니다.');
  }
...
export async function callLLM(
  ...
  switch (provider) {
    ...
    case 'NVIDIA':
      return callNvidia(prompt, maxTokens, apiKeyOverride);
    case 'ANTHROPIC':
    default:
