...
## 2026-09-07 — 프로젝트 생성 화면 개편 (개요/자유 역할 리스트/위키·간편모드 스위치)

- **데이터 모델**: `Project`에 `description`(개요), `wikiEnabled`(기본 true), `simpleMode`(기본 false) 추가. 신규 모델 `ProjectRole`(projectId, label 자유텍스트, userId?, userName?, order) — PM(`projectManagerName`)만 고정 필드로 유지하고 PL을 포함한 나머지 역할은 전부 자유 리스트로 전환(예: PL/기획리더/디자인리더/퍼블리싱리더/개발리더/시장조사리더 등 업종에 맞게 자유 입력·추가/삭제).
- **API**: `POST /api/projects`, `PATCH /api/projects/[id]`에서 개요/위키사용여부/간편모드/역할 리스트 처리(역할은 PATCH 시 전체 삭제 후 재생성 방식). `GET`/생성/수정 응답 모두 `roles` 관계 포함.
- **화면**(`src/app/projects/page.tsx`): PM 입력은 유지, PL 고정칸은 제거하고 "역할" 섹션(자주쓰는 역할 빠른입력 칩 + 자유 추가/삭제 리스트, 담당자는 팀원선택 또는 이름 직접입력)으로 교체. "위키 사용"/"간편모드" 체크박스 추가 — 간편모드 체크 시 개요·역할 섹션이 숨겨져 이름만 입력하면 바로 생성 가능. 프로젝트 목록 카드에 개요/역할/간편모드 뱃지 표시, 위키 미사용 프로젝트는 멤버 패널에서 "위키" 버튼 숨김.
...
## 2026-09-08 (4차) — 업무 목록 표 컬럼 너비 자동 계산 + 작업시간 컬럼 선택적 노출

표 컬럼 너비가 행마다 들쑥날쑥하다는 피드백에 따라 `src/app/tasks/page.tsx`/`tasks.module.css` 개선.

- **기본 컬럼 고정폭(PC 기준)**: 체크박스 36px, ID 56px, 담당자 110px, 등록일자 110px, 목표일 110px, 상태 110px(합 532px)은 항상 고정. 나머지 폭은 **제목**과 **비고**가 정확히 절반씩(`calc((100% - 고정폭) / 2)`) 나눠 가져 표가 항상 100%를 채우도록 함(`table-layout: fixed` + CSS 변수 `--fixed-cols-width`로 JS에서 실시간 계산).
...
## 2026-09-09 — 프로젝트 관리 화면 우측 멤버 패널 제거

프로젝트에 팀원을 추가해도 우측 멤버 패널에 반영되지 않는다는 신고 — 실제로는 새로고침 버그가 아니라 "역할(예: 기획리더) 지정"과 "멤버 추가"가 서로 다른 개념이라 발생한 혼동으로 보였음(역할만 지정하고 별도 "멤버 추가" 드롭다운으로는 추가하지 않은 경우 우측 패널에 안 나타나는 게 정상 동작). 우측 패널이 프로젝트 멤버 추가/삭제의 유일한 진입점이라는 점을 안내했으나, 사용자가 그래도 삭제를 요청해 그대로 반영.
...
## 2026-09-09 (5차) — 직전 변경 되돌림: PM/역할 드롭다운은 조직 전체 팀원이 맞음

바로 이전 라운드(9f7921d)에서 "PM/역할 드롭다운을 프로젝트 멤버로 제한"한 것은 방향이 반대였음을 확인 — 되돌림.

- **올바른 설계**: 프로젝트 수정 화면의 PM/역할 드롭다운은 "이 프로젝트에 새로 투입할 사람을 조직 전체에서 고르는" 용도이므로 조직 전체 팀원(`allUsers`)이 보여야 함. 반대로 업무 등록 화면의 담당자 드롭다운은 "이미 이 프로젝트에 투입된 사람 중에서 고르는" 용도이므로 프로젝트 멤버로 제한되어야 함(이건 원래부터 그렇게 되어 있었고 변경 없음).
- 직전 변경으로 프로젝트 멤버 드롭다운을 `editingProject.members`로 제한해버려, 아직 멤버가 아닌 사람은 애초에 역할 드롭다운에 뜨지도 않아 영원히 멤버가 될 수 없는 악순환이 생겼음(한지아님 사례). `src/app/projects/page.tsx`의 `memberOptions` 계산을 제거하고 두 드롭다운 모두 `allUsers`로 원복.
- `npx tsc --noEmit` 오류 0개, `npx next build` 성공.

## 2026-09-09 (6차) — 업무 목록의 담당자 변경 드롭다운도 프로젝트 멤버로 제한

업무 목록 화면(각 프로젝트 섹션의 행)에서 담당자를 바꾸는 드롭다운이 조직 전체 사용자를 보여주고 있어("클로드"처럼 그 프로젝트와 무관한 사람도 노출), 요청하신 "업무는 프로젝트에 투입된 사람과만 진행하므로 나머지는 보이면 안 된다" 원칙에 어긋났음. 지난 라운드에서 ADMIN 제외 규칙을 없애며 `/users`(조직 전체) 조회로 바꾼 게 원인.

- `src/app/tasks/page.tsx`: `ProjectMeta`에 `members` 추가(`GET /api/projects` 응답에 이미 포함되어 있던 필드를 타입에 반영). `ProjectTaskSection` 내부에서 `project.members`가 있으면 그 프로젝트 멤버만, 없으면(미지정 업무 섹션) 기존처럼 조직 전체 목록을 담당자 후보로 사용하도록 `projectAssignableWorkers` 계산 추가. 업무 행의 담당자 변경 셀렉트와 "새 업무" 인라인 등록 폼의 담당자 셀렉트 모두 이 값을 사용하도록 수정.
- 상단 필터바의 "일괄 담당자 변경" 셀렉트(여러 프로젝트에 걸친 업무를 한 번에 선택할 수 있음)는 특정 프로젝트로 범위를 좁힐 수 없어 조직 전체 목록을 그대로 유지.
- `npx tsc --noEmit` 오류 0개, `npx next build` 성공.

## 2026-09-10 — 모바일 UI 최적화 (업무 등록/대시보드/헤더)

⚠️ 이 환경은 실제 DB 연결이 없어 로그인 후 실제 화면을 모바일 뷰포트로 직접 렌더링해 확인하지 못했음 — 코드 레벨 반응형 CSS 검토 및 수정만 진행. 배포 후 실제 휴대폰/모바일 브라우저에서 확인 필요.

- 기존에 이미 업무 목록(카드형 전환), 업무 상세, 대시보드, 헤더(햄버거 메뉴)에 640px/768px 기준 반응형 처리가 되어 있었음을 확인. 그 위에 아래 항목 보완:
  - `tasks/create/create.module.css`: 모바일에서 페이지 좌우 여백을 32px→줄임(12~16px), "업무 등록/취소" 버튼을 세로로 쌓아 전체 너비로 노출.
  - `dashboard/dashboard.module.css`: "나의 업무"/"최근 활동" 목록 항목이 좁은 화면에서 제목+상태+날짜가 한 줄에 눌리던 것을 제목을 다음 줄로 내려 2줄 레이아웃으로 전환. 상단 바로가기 버튼(업무 배정/통계 조회 등)을 2열 그리드로 배치.
...
## 2026-09-14 (4차) — 랜딩 페이지를 잔디(Jandi) 스타일 참고해 개편

사용자가 공유한 잔디 AI 랜딩 페이지 스크린샷(그라디언트 히어로 + 떠있는 말풍선형 장식 + 앱 스크린샷 위에 겹쳐진 AI 결과 카드, 그리고 아이콘 배지 + 굵은 제목 + 설명 + 예시 질문 칩 + 반대편 스크린샷으로 구성된 좌우 교차 기능 소개 섹션)을 참고해 하이파이브 랜딩 페이지(`src/app/page.tsx`, `landing.module.css`) 개편. jandi.com 원본은 네트워크 정책상 직접 접속 불가해 스크린샷 기준으로만 반영.

- **히어로**: 단색 배경 → 하이파이브 브랜드 색(잉크 그린→코랄→골드) 대각선 그라디언트로 변경. "회의록 자동 요약", "오늘 할 일 3건", "D-2 마감 알림" 떠있는 말풍선 배지 3개 추가. 실제 스크린샷이 없어 업무 목록을 흉내낸 CSS 목업 창(사이드바+행 목록)을 그려 넣고, 그 위에 겹치는 "주간 보고서 AI" 결과 카드(플로팅) 배치.
- **좌우 교차 기능 소개 2개 섹션** 신규: "AI 자동화"(주간 보고서 AI 카드 목업 + 예시 질문 칩 2개), "프로젝트 협업"(캘린더 목업 + 예시 질문 칩 2개). 각 섹션은 배지(eyebrow)+굵은 제목+설명+예시 칩 구조.
- 기존 6개 기능 카드는 스포트라이트에 옮긴 2개(AI 자동화, 프로젝트 협업)를 빼고 나머지 3개(업무 관리/알림&보안/외부 연동)만 압축 그리드로 유지.
...
## 2026-09-14 (6차) — 히어로 배경을 색 없는 흰 배경으로 전면 변경

그라디언트 톤다운 후에도 "여전히 부담스럽고 이상하다"는 피드백 — 색 배경 자체를 없애고 페이지 나머지와 동일한 흰/밝은 배경(`--bg-base`)으로 통일하기로 결정(사용자 확인).

- 히어로 배경을 그라디언트에서 `var(--bg-base)`로 변경, 제목/부제/배지 색상을 원래의 어두운 텍스트(`--text-primary`/`--text-secondary`)와 액센트 컬러(`--accent-light`/`--accent`) 조합으로 되돌림.
- 떠있는 말풍선 배지(`.heroBubble`)를 반투명 흰색(어두운 배경 전제)에서 흰 카드+테두리+그림자 스타일로 변경.
...
## 2026-09-18 (11차) — 라운드 14: 데이터 이전/내보내기 (로드맵 마지막 라운드)

- **팀원 일괄 등록**: `POST /api/users/import`(ADMIN 전용, xlsx: 이메일/이름/역할/소속그룹) — 기존 `/api/users` POST와 동일하게 임시 비밀번호 발급. `/users` 페이지에 "📤 일괄 등록(xlsx)" 버튼 추가.
- **업무 엑셀 가져오기**: `POST /api/tasks/import`(ADMIN/LEADER, xlsx: 제목/담당자(이름·이메일)/목표일/프로젝트/우선순위/비고) — 담당자·프로젝트는 이름 또는 이메일로 매칭. `/tasks` 페이지에 버튼 추가.
- **프로젝트 엑셀 가져오기**: `POST /api/projects/import`(ADMIN/LEADER, xlsx: 프로젝트명/설명/PM이름), 중복 이름은 건너뜀. `/projects` 페이지에 버튼 추가.
- **통계 엑셀 내보내기**: 확인해보니 `/stats` 페이지에 이미 클라이언트에서 `XLSX.writeFile`로 월간요약+작업자별부하량을 내보내는 기능이 구현되어 있어(서버 API 없이 클라이언트에서 직접 생성) 중복 구현하지 않음(처음 만들었던 `/api/stats/export`는 삭제).
- **비용 자료 내보내기**: 라운드12에서 법인카드는 이미 다운로드 추가됨. 이번엔 간편장부(`GET /api/expenses/ledger/export`)도 대칭으로 추가, `/expenses` 화면에 다운로드 버튼 추가.
- **조직 데이터 백업**: 확인 결과 `/settings/organization` 페이지의 "전체 데이터 백업 다운로드" 버튼과 `GET /api/settings/organization/backup`(조직/사용자/프로젝트/업무/타임로그/댓글/히스토리/첨부파일메타/FAQ/공지/신청/위키/스티키노트/개인문서/외부연동 설정까지 포괄하는 JSON 백업, 감사 로그 기록 포함)이 이미 구현되어 있었음 — 처음에 이 사실을 모르고 더 축소된 버전으로 덮어썼다가(파일을 먼저 읽지 않고 새로 작성하는 실수) 되돌림. 추가 작업 없이 기존 구현 그대로 유지.
- **삭제 데이터 복구는 이번에도 보류**: 현재 삭제는 전부 하드 삭제(예: `DELETE /api/users/[id]?hard=true`, 업무/프로젝트 삭제 등)라, 복구 기능을 만들려면 주요 모델 전반에 `deletedAt`(소프트 삭제) 컬럼과 조회 시 필터링, 복구 API, 휴지통 UI가 필요한 스키마 전역 영향 작업이다. 이번 로드맵 라운드 범위를 넘어서는 별도 과제로 남김.
