...
/manual         → 인증 필요 (전 역할, 정적 사용 메뉴얼)
/login          → 누구나 접근 가능
/register       → 누구나 접근 가능 (조직 가입, 이용약관/개인정보처리방침 동의 필수)
/legal/terms    → 누구나 접근 가능 (이용약관, 정적 페이지)
/legal/privacy  → 누구나 접근 가능 (개인정보처리방침, 정적 페이지)
...
```
User (1) ──< Task [registrant / worker] (1) ──< TimeLog
User (1) ──< InfoItem
User (1) ──< TaskHistory
...
| registrantId / workerId | Int FK (registrantId는 DB 컬럼명 plannerId 그대로 매핑) | 등록자 / 작업자 |
...
프로젝트별 커스텀 필드(노션식 자유 속성). `ProjectField`는 정의(`projectId`, `name`, `type`: TEXT/NUMBER/DATE/SELECT/CHECKBOX/LINK, `options`: SELECT일 때 콤마구분 선택지, `order`, `@@unique([projectId, name])`, 프로젝트당 최대 10개). LINK 타입은 값 입력을 `type="url"` input으로 받고, 조회 시 클릭 가능한 `<a target="_blank">` 링크로 렌더링된다(별도 커스텀 필드 속성으로 여러 개 둘 수 있다는 점에서 `Task.externalLink` 단일 필드와 다름), `TaskFieldValue`는 업무별 값(`taskId`, `fieldId`, `value`: 모든 타입을 문자열로 저장, `@@unique([taskId, fieldId])`). `ProjectStatus`와 동일한 "프로젝트 단위 정의 테이블 + 값 테이블" 패턴이며, 상태와 달리 기본값 폴백은 없음(필드가 없으면 그냥 컬럼이 없는 것). `/api/projects/[id]/fields`(GET/PUT, PUT은 ADMIN/LEADER)와 `/api/tasks/[id]/fields`(PUT, 값 upsert)로 관리하고, `useProjectFields` 훅이 조회/저장을 감싼다. 업무 목록(`/tasks`)은 프로젝트별로 독립된 섹션("OOO 리스트")으로 나뉘어 표시되며(각 섹션은 `ProjectTaskSection` 컴포넌트, 자체 `useProjectFields`), 각 섹션 헤더에 항상 "+ 속성 추가" 버튼이 바로 보이고 컬럼 헤더의 ✕ 버튼으로 삭제, 헤더를 드래그해서 순서 변경도 가능하다(노션 스타일). 각 섹션 테이블 맨 아래에는 "+ 새 업무" 행이 있어 클릭하면 제목/담당자/목표일을 한 번에 입력하는 인라인 폼으로 바뀌고 Enter(또는 "추가" 버튼)하면 페이지 이동 없이 즉시 업무가 생성된다(담당자 기본값은 현재 로그인 사용자, 목표일은 선택 사항 — 프로젝트 미지정 섹션에는 노출 안 함). 링크(`externalLink`)는 이 인라인 등록 단계에는 없고 등록 후 업무 상세(`/tasks/[id]`)에서 연결한다. 프로젝트 미지정 업무는 맨 아래 "미지정 업무" 섹션으로 분리. 업무 상세(`/tasks/[id]`)에도 "속성" 카드로 동일하게 반영되어 있다(필드가 없으면 카드 자체가 숨겨짐). 칸반 노출(Phase 3)은 진행하지 않기로 결정됨 — `docs/개선1.0.md` 참고. 640px 이하 모바일에서는 이 테이블이 순수 CSS로 카드형으로 전환된다(각 `<td>`의 `data-label` 속성을 `::before`로 라벨 표시, 헤더 행은 숨김 — `docs/DESIGN.md` §8 참고).
...
### UserNotification
인앱 알림. `type`(WORKER_ASSIGNED / WORKER_CHANGED / STATUS_CHANGED / REVIEW_REQUESTED), `isRead`. 업무 상태가 **어떤 값으로든** 바뀌면(`src/lib/notify.ts`의 `notifyStatusChanged`) 상태를 변경한 사람 본인을 제외한 등록자(registrant)·담당자(worker) 모두에게 생성되고, 담당자 변경 시에도(`notifyWorkerChange`) 새 담당자+등록자에게 생성된다. 현재는 상시 노출 UI(벨 아이콘)는 없고, `src/hooks/useNotifications.ts`가 30초 폴링으로 안읽음 개수 증가를 감지하면 `src/components/common/NotificationToast.tsx`가 화면 우하단에 토스트로 띄운다(`src/components/Providers.tsx`에 전역 배치, 로그인 상태에서만 동작).
...
- **업무 목록**(`/tasks`)의 상태 필터/행별 상태 변경 셀렉트도 각 업무가 속한 프로젝트의 상태 목록을 사용(`useProjectStatuses` 훅으로 `GET /api/projects/statuses` 일괄 조회 후 클라이언트에서 매핑). 상단 "상태" 필터 옵션은 전체 업무가 아니라 **현재 선택된 프로젝트/담당자 필터가 반영된 범위**에서 실제 사용 중인 코드만 모아 구성한다(프로젝트를 바꿨는데 다른 프로젝트의 상태가 섞여 보이던 문제 수정, 2026-07-01 30차) — 프로젝트를 바꾸면 상태 필터도 자동으로 초기화된다.
- 필터 바 맨 앞의 "프로젝트" 드롭다운과 칸반 보드(`/tasks/kanban`)의 프로젝트 선택, 업무 등록(`/tasks/create`)의 프로젝트 선택, 위키 허브(`/wiki`)는 모두 `GET /api/projects`를 사용한다. 이 API는 ADMIN이 아니면 `ProjectMember` 소속 프로젝트 **+ 본인이 workerId/registrantId로 연결된 업무가 있는 프로젝트**를 합쳐서 반환한다(2026-07-01 30차 수정) — 이전에는 멤버 소속만 봤기 때문에, 프로젝트에 정식 멤버로 등록되지 않은 채 업무만 배정받은 WORKER는 해당 프로젝트가 각종 드롭다운에 아예 노출되지 않았다(업무 목록 자체에는 보이는데도 전환/필터 대상에서는 빠지는 불일치). `GET /api/projects/statuses`(bulk)도 동일한 원칙(멤버 + 업무 배정)으로 조회한다.
- 단, 위키(`/api/projects/[id]/wiki`의 `checkAccess`)는 이 확장을 적용하지 않고 여전히 `ProjectMember` 소속만으로 접근을 제한한다 — "소속 멤버만 위키를 본다"는 원래 요구사항이었으므로 의도적으로 다르게 유지.
