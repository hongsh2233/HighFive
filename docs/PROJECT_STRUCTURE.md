...
- 필터 바 맨 앞의 "프로젝트" 드롭다운과 칸반 보드(`/tasks/kanban`)의 프로젝트 선택, 업무 등록(`/tasks/create`)의 프로젝트 선택, 위키 허브(`/wiki`)는 모두 `GET /api/projects`를 사용한다. 이 API는 ADMIN이 아니면 `ProjectMember` 소속 프로젝트 **+ 본인이 workerId/plannerId로 연결된 업무가 있는 프로젝트**를 합쳐서 반환한다(2026-07-01 30차 수정) — 이전에는 멤버 소속만 봤기 때문에, 프로젝트에 정식 멤버로 등록되지 않은 채 업무만 배정받은 WORKER는 해당 프로젝트가 각종 드롭다운에 아예 노출되지 않았다(업무 목록 자체에는 보이는데도 전환/필터 대상에서는 빠지는 불일치). `GET /api/projects/statuses`(bulk)도 동일한 원칙(멤버 + 업무 배정)으로 조회한다.
