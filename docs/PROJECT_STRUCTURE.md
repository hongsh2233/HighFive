...
- **업무 목록**(`/tasks`)의 상태 필터/행별 상태 변경 셀렉트도 각 업무가 속한 프로젝트의 상태 목록을 사용(`useProjectStatuses` 훅으로 `GET /api/projects/statuses` 일괄 조회 후 클라이언트에서 매핑). 필터 바 맨 앞의 "프로젝트" 드롭다운은 별도 API 호출 없이 이미 로드된 `tasks` 배열에서 `task.project`를 모아 구성한다(담당자 필터와 동일한 패턴) — `GET /api/projects`(멤버 소속 기준)를 쓰지 않는 이유는 WORKER의 업무 목록이 프로젝트 소속 여부와 무관하게 배정된 업무를 전부 보여주기 때문에, 소속 API 기준으로 만들면 실제 보이는 업무의 프로젝트와 드롭다운 목록이 어긋날 수 있어서다.
- `GET /api/projects/statuses`(bulk)도 같은 이유로 ADMIN이 아니면 `ProjectMember` 소속 프로젝트 + 본인이 workerId/plannerId로 연결된 업무의 프로젝트를 합쳐서 조회한다(2026-07-01 29차 수정 — 이전에는 멤버 소속만 봐서 비소속 WORKER의 업무에 커스텀 상태가 반영되지 않는 버그가 있었음).
